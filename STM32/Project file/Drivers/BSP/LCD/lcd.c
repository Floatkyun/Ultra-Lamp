...
//    /* 初始化完成以后,提速 */
    ...
    {
        /* 重新配置写时序控制寄存器的时序 */
        fsmc_write_handle.AddressSetupTime = 1; /* 地址建立时间(ADDSET)为3个fsmc_ker_ck=6*3=18ns */
        fsmc_write_handle.DataSetupTime = 1;    /* 数据保持时间(DATAST)为3个fsmc_ker_ck=6*3=18ns */
...
{
	uint16_t color_r=color&0x001F;
	uint16_t color_g=(color&0x07E0)>>5;
	uint16_t color_b=(color&0xF800)>>11;
 return color_r<<11|color_g<<5|color_b;
}
...
void lcd_clear(uint16_t color)
{
    ...
    {
        LCD->LCD_RAM = BGR565toRGB565(color);
    }
}
...
void lcd_color_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t *color)
{
    ...
    for (i = 0; i < height; i++)
    {
        ...
        for (j = 0; j < width; j++)
        {
            LCD->LCD_RAM = BGR565toRGB565(color[i * width + j]); /* 写入数据 */
        }
    }
}
